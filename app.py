...
if page == "Dashboard":
    ...
    # Use RPC functions to get aggregated numbers without pulling all rows
    if branch_id:
    ...
    # Alert counts (still needs alert_log table, but it's small)
...
# ============================================================
    ...
    total = query.execute(count="exact").count
    total_pages = (total + PAGE_SIZE - 1) // PAGE_SIZE
...
# ============================================================
    ...
    query = supabase.table("alert_log").select("*, products(name), branches(name)").order("created_at", desc=True)
    if branch_id:
        query = query.eq("branch_id", branch_id)
    total = query.execute(count="exact").count
    total_pages = (total + PAGE_SIZE - 1) // PAGE_SIZE
    ...
    # Manual action update (for unactioned alerts)
                ...
                if other_days < 7 or other_agg['total_qty'] < other_rp:
                    ...
                    if transfer_qty > 0:
                        suggestions.append({
                            ...
                            'product_name': f"Product {p_id}",  # we can lookup name but keep simple
