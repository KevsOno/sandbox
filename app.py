...
def upload_csv_to_table(table_name, df, extra_columns={}):
    ...
    try:
        ...
        return False, f"Upload failed at chunk {i//chunk_size+1}/{total_chunks}. No data committed. Error: {e}"

# ---------- PAGINATION COUNT CACHING ----------
def get_cached_count(table_name, filter_col=None, filter_val=None, key_suffix=""):
    cache_key = f"count_{table_name}_{key_suffix}"
    if cache_key not in st.session_state or st.session_state.get(f"{cache_key}_refresh"):
        query = supabase.table(table_name).select("*", head=True, count="exact")
        if filter_col and filter_val:
            query = query.eq(filter_col, filter_val)
        st.session_state[cache_key] = query.execute().count
        if f"{cache_key}_refresh" in st.session_state:
            del st.session_state[f"{cache_key}_refresh"]
    return st.session_state[cache_key]

def refresh_count(table_name, key_suffix=""):
    cache_key = f"count_{table_name}_{key_suffix}"
    st.session_state[f"{cache_key}_refresh"] = True
    st.rerun()

# ============================================================
# PAGE: DASHBOARD (RPC aggregates – unchanged)
# ============================================================
if page == "Dashboard":
...
# ============================================================
# PAGE: BRANCHES (unchanged, but uses explicit columns)
# ============================================================
elif page == "Branches":
    if st.session_state.user_role != "admin":
...
# ============================================================
    ...
    total = get_cached_count("products", key_suffix="products")
    ...
    with col1:
        if st.button("Prev", disabled=st.session_state.prod_page==0):
            st.session_state.prod_page -= 1
            st.rerun()
    with col2:
        if st.button("Next", disabled=st.session_state.prod_page>=total_pages-1):
            st.session_state.prod_page += 1
            st.rerun()
    with col3:
        if st.button("🔄 Refresh count"):
            refresh_count("products", "products")
    ...
    with st.form("add_product"):
        ...
        if st.form_submit_button("Add"):
            if not sku or not name:
                ...
                # Invalidate count cache for products
                if "count_products_products" in st.session_state:
                    del st.session_state["count_products_products"]
                st.rerun()
...
# ============================================================
    ...
    # Build count query on the underlying inventory table (still needed)
    count_key = f"inventory_{branch_id}" if branch_id else "inventory_all"
    total = get_cached_count("inventory", filter_col="branch_id" if branch_id else None,
                             filter_val=branch_id if branch_id else None, key_suffix=count_key)
    total_pages = (total + PAGE_SIZE - 1) // PAGE_SIZE

    # Use the view for data (explicit columns)
    ...
    col1, col2, col3 = st.columns([1,1,1])
    with col1:
        if st.button("Prev Inv", disabled=st.session_state.inv_page==0):
            st.session_state.inv_page -= 1
            st.rerun()
    with col2:
        if st.button("Next Inv", disabled=st.session_state.inv_page>=total_pages-1):
            st.session_state.inv_page += 1
            st.rerun()
    with col3:
        if st.button("🔄 Refresh count"):
            refresh_count("inventory", count_key)
        ...
        if upload_type == "Inventory (current stock)":
            ...
            if st.button("Upload Inventory"):
                ...
                if success:
                    ...
                    # Invalidate inventory count cache
                    count_key = f"inventory_{selected_branch_id}" if selected_branch_id else "inventory_all"
                    if f"count_inventory_{count_key}" in st.session_state:
                        del st.session_state[f"count_inventory_{count_key}"]
                    st.rerun()
                else:
                    st.error(err)
        else:
            ...
            if st.button("Upload Movements"):
                ...
                if success:
                    ...
                    # No count cache for movements – fine
                else:
                    st.error(err)
...
# ============================================================
    ...
    count_key = f"alert_log_{branch_id}" if branch_id else "alert_log_all"
    ...
    col1, col2, col3 = st.columns([1,1,1])
    with col1:
        if st.button("Prev Alerts", disabled=st.session_state.alert_page==0):
            st.session_state.alert_page -= 1
            st.rerun()
    with col2:
        if st.button("Next Alerts", disabled=st.session_state.alert_page>=total_pages-1):
            st.session_state.alert_page += 1
            st.rerun()
    with col3:
        if st.button("🔄 Refresh count"):
            refresh_count("alert_log", count_key)
...
# ============================================================
    ...
    count_key = f"stock_limits_{branch_id}" if branch_id else "stock_limits_all"
    ...
    col1, col2, col3 = st.columns([1,1,1])
    with col1:
        if st.button("Prev Limits", disabled=st.session_state.limits_page==0):
            st.session_state.limits_page -= 1
            st.rerun()
    with col2:
        if st.button("Next Limits", disabled=st.session_state.limits_page>=total_pages-1):
            st.session_state.limits_page += 1
            st.rerun()
    with col3:
        if st.button("🔄 Refresh count"):
            refresh_count("stock_limits", count_key)
...
# ============================================================
    ...
    **Risk Levels:** LOW 🟢 → MODERATE 🟡 → HIGH 🟠 → CRITICAL 🔴
    ...
    count_key = f"product_risk_scores_{branch_id}" if branch_id else "product_risk_scores_all"
    ...
    col1, col2, col3 = st.columns([1,1,1])
    with col1:
        if st.button("Prev Risk", disabled=st.session_state.risk_page==0):
            st.session_state.risk_page -= 1
            st.rerun()
    with col2:
        if st.button("Next Risk", disabled=st.session_state.risk_page>=total_pages-1):
            st.session_state.risk_page += 1
            st.rerun()
    with col3:
        if st.button("🔄 Refresh count"):
            refresh_count("product_risk_scores", count_key)
...
# ============================================================
# PAGE: TRANSFER SUGGESTIONS (unchanged, uses view)
# ============================================================
