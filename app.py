...
# Helper to reset all pagination indices when branch changes
...
def upload_csv_to_table(table_name, df, extra_columns={}):
    ...
    try:
        ...
        # If chunk fails, warn the user – no automatic retry to avoid duplicates
        error_msg = f"Upload failed at chunk {chunk_num}/{total_chunks}. No data was committed for this chunk. Please check your data and retry. Error: {e}"
        return False, error_msg

# ============================================================
# PAGE: DASHBOARD (RPC aggregates)
# ============================================================
if page == "Dashboard":
...
# ============================================================
    ...
    total_res = supabase.table("products").select("*", head=True, count="exact").execute()
...
# ============================================================
    ...
    count_query = supabase.table("inventory").select("*", head=True, count="exact")
    if branch_id:
...
# ============================================================
# PAGE: CSV UPLOAD (with user guide and safe chunked upload)
# ============================================================
    ...
    if upload_type == "Inventory (current stock)":
        ...
        **Sample row:**  
        ...
        `SKU12345,BATCH-001,50,2026-12-31,warehouse`
        """)
        ...
        **Sample row:**  
        ...
        `SKU12345,-3,2026-05-17,Sold to customer A`
        """)
...
# ============================================================
    ...
    count_query = supabase.table("alert_log").select("*", head=True, count="exact")
    if branch_id:
        count_query = count_query.eq("branch_id", branch_id)
    total = count_query.execute().count
    total_pages = (total + PAGE_SIZE - 1) // PAGE_SIZE
...
# ============================================================
    ...
    count_query = supabase.table("stock_limits").select("*", head=True, count="exact")
    if branch_id:
        count_query = count_query.eq("branch_id", branch_id)
    total = count_query.execute().count
    total_pages = (total + PAGE_SIZE - 1) // PAGE_SIZE
...
# ============================================================
# PAGE: RISK & FEFO (database sorting + pagination + empty check)
# ============================================================
elif page == "Risk & FEFO":
    ...
    **Risk Levels:** LOW 🟢 → MODERATE 🟡 → HIGH 🟠 → CRITICAL 🔴
    ...
    # Lightweight count
    count_query = supabase.table("product_risk_scores").select("*", head=True, count="exact")
    if branch_id:
        count_query = count_query.eq("branch_id", branch_id)
    total = count_query.execute().count
    total_pages = (total + PAGE_SIZE - 1) // PAGE_SIZE
    
    # Sort selection before query
    sort_by = st.selectbox("Sort by", ["risk_score desc", "expiry_date asc", "financial_value desc"])
    
    data_query = supabase.table("product_risk_scores").select("*, products(name, sku), branches(name)")
    if branch_id:
        data_query = data_query.eq("branch_id", branch_id)
    
    if sort_by == "risk_score desc":
        data_query = data_query.order("risk_score", desc=True)
    elif sort_by == "expiry_date asc":
        data_query = data_query.order("expiry_date", desc=False)
    else:
        data_query = data_query.order("financial_value", desc=True)
    
    risk_scores = data_query.range(offset, offset+PAGE_SIZE-1).execute().data
...
# ============================================================
# PAGE: TRANSFER SUGGESTIONS (database view + defensive empty check)
# ============================================================
